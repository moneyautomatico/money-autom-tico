...
app.post('/register', async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: 'Usuário criado!', user: { id: user._id, name: user.name, email: user.email } });
  } catch (err) {
    console.error(err);
...
app.post('/login', async (req, res) => {
  try {
    ...
    return res.json({ message: 'Login realizado!', token, user: { id: user._id, name: user.name, email: user.email, iaResumo: user.iaResumo, baseAprendizado: user.baseAprendizado } });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ error: 'Erro interno.' });
  }
...
app.post('/leads', autenticar, async (req, res) => {
  try {
    ...
    if (err.code === 11000) return res.status(409).json({ error: 'Telefone já cadastrado.' });
    return res.status(500).json({ error: 'Erro interno.' });
  }
});
...
app.post('/whatsapp/enviar', autenticar, async (req, res) => {
  ...
  if (!whatsappReady) return res.status(503).json({ error: 'WhatsApp não está conectado.' });
  try {
...
app.post('/disparar', autenticar, async (req, res) => {
  ...
  if (!numeros || !mensagem) return res.status(400).json({ error: 'Informe números e mensagem.' });
  if (!whatsappReady) return res.status(503).json({ error: 'WhatsApp ainda não está conectado.' });
  const lista = numeros.split('\n').map(n => n.trim()).filter(Boolean);
  if (!lista.length) return res.status(400).json({ error: 'Nenhum número válido informado.' });
...
app.get('/progresso', autenticar, (req, res) => {
  return res.json({ status: disparo.status, atual: disparo.atual, total: disparo.total, pausado: disparo.pausado });
});
